...
class DBWNode(object):
    ...
    def loop(self):
        rate = rospy.Rate(17) # must be submitted with 50Hz
        while not rospy.is_shutdown():
            # TODO: Get predicted throttle, brake, and steering using `twist_controller`
            # You should only publish the control commands if dbw is enabled

            # Project Implementation ( support code - udacity classroom )
            if not None in (self.current_vel,self.current_angular_vel,self.dbw_enabled ,self.linear_vel, self.angular_vel):
